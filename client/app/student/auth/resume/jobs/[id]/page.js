"use client";
import { asyncediteducation, asynceditjob } from "@/store/Actions/studentActions";
...
const page = ({ params }) => {
  ...
  // console.log(student)
 const index = (student && student.resume.jobs.findIndex(
    (i) => i.id === params.id
  ))
 const mainstudent = (student && student.resume.jobs[index])
...
const [description, setDescription] = useState(student && mainstudent.description)
  ...
  const EditJobHandler = () => {
    ...
    dispatch(asynceditjob(params.id, job));
    ...
    <div className="container mt-5  flex justify-center">
      <Card color="transparent" shadow={false}>
        ...
        <form className="mt-8 mb-2 w-80 max-w-screen-lg sm:w-96">
          ...
          <Button onClick={EditJobHandler} className="mt-6" fullWidth>
            Update
          </Button>
        </form>
      </Card>
